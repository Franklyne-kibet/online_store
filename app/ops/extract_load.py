...
def load_customer_risk_score(customer_risk_score: List[Dict[str, int]]):
    ...
    """
    with WarehouseConnection(get_aws_creds()).managed_cursor() as curr:
...
def extract_orders_data(context) -> List[Dict[str, str]]:
    ...
    objs = s3.list_objects_v2(Bucket=context.op_config["orders_bucket_name"])["Contents"]
    ...
    for line in data.split("\n")[:-1]:
        order_id, customer_id, item_id, item_name, delivered_on = str(line).split(",")
        orders.append(
            {
                "order_id": order_id,
